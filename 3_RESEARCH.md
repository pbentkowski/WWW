...
We gathered data from Statistics Poland (GUS) and its 2021 National Census, Ministry of Health, and academic literature in a handy SQL database.
...
Homophily is a sociological concept suggesting that individuals who share similar traits—such as age, gender, race, religion, occupation, worldview, or political beliefs—are more likely to be drawn to and form bonds with one another. With [Tomasz Gubiec](https://scholar.google.com/citations?user=KTmLUegAAAAJ&hl=en) we studied how potential divisions between compliance and non-compliance with measures designed to limit the spread of infectious diseases can influence the dynamics of an epidemic. Specifically, we aim to explore how the extent of such divisions affects the two groups, particularly in terms of the number of infected individuals within each group. See [the model and resulting paper](https://codeberg.org/pbentkowski/homophily_epidemic).
...
My role includes modifying the C++11 model, preparing data, calibrating parameters, and automating simulations with Python scripting. Synthetic population generated by [the model was used in COVID-19 studies](https://www.science.org/doi/10.1126/sciadv.abd8750).
